...
public final class Program1GeneratedCode1 extends Program1 {
    ...
    private static void generateCodeForStatement(Statement s,
        ...
        switch (s.kind()) {
            case BLOCK: {
                ...
                int pos = 0;
                while (pos < s.lengthOfBlock()) {
                    Statement substatement = s.removeFromBlock(pos);
                    generateCodeForStatement(substatement, context, cp);
                    s.addToBlock(pos, substatement);
                    pos++;
                }
            ...
            case CALL: {
                ...
                if (instruction.equals("move") || instruction.equals("turnleft")
                    ...
                    bytecode = Program.Instruction
                            .valueOf(instruction.toUpperCase()).byteCode();
                    cp.add(cp.length(), bytecode);
                } else if (context.hasKey(instruction)) {
                    Map.Pair<String, Statement> userInstruction = context
                            .remove(instruction);
                    Statement block = userInstruction.value();
                    generateCodeForStatement(block, context, cp);
                    context.add(userInstruction.key(), userInstruction.value());
                } else {
                    Reporter.fatalErrorToConsole(
                            "Recursion or instruction not in context");
                }
    ...
    public Sequence<Integer> generatedCode() {
        ...
        cp.add(cp.length(), Program.Instruction.HALT.byteCode());
