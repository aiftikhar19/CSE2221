...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private static int mod(int a, int b) {
        ...
        //Counterclockwise direction for negative numbers
        if (remainder < 0) {
            modulus = b + remainder;
        } else {
    ...
    private void createNewRep(int hashTableSize) {
        //Create hashTable and initialize buckets
        this.hashTable = new Array1L<Map<K, V>>(hashTableSize);
        for (int index = 0; index < hashTableSize; index++) {
            this.hashTable.setEntry(index, new Map2<K, V>());
        }
        this.size = 0;
    }
    ...
    public final void add(K key, V value) {
        ...
        //Find appropriate bucket number for key
        int keyHash = key.hashCode();
        int bucket = mod(keyHash, this.hashTable.length());

        //Add key and value to appropriate bucket and update $this.size
    ...
    public final Pair<K, V> remove(K key) {
        ...
        //Remove pair from bucket and update $this.size
    ...
    public final Pair<K, V> removeAny() {
        ...
        //Find a bucket whose length !=0 to remove a pair from
        int bucketToRemove = 0;
        while (this.hashTable.entry(bucketToRemove).size() == 0) {
            bucketToRemove++;
        }

        //Remove a Map.Pair from nonempty bucket and update $this.size
        Map.Pair<K, V> removed = this.hashTable.entry(bucketToRemove)
                .removeAny();
        this.size--;
    ...
    public final boolean hasKey(K key) {
        ...
        // Return true/false whether the bucket contains the key
