...
public final class ArraySiftDownMain {
    ...
    private static void siftDown(Array<Integer> array, int top, int last) {
        ...
        boolean hasLeft = leftIndex <= last;
        boolean hasRight = rightIndex <= last;

        //Only need to siftRootDown if root has any child heaps
        int root = array.entry(top);
        if (hasRight && hasLeft) {
            int leftHeap = array.entry(leftIndex);
            int rightHeap = array.entry(rightIndex);
            ...
            if (root > array.entry(smallestChildIndex)) {
                array.exchangeEntries(top, smallestChildIndex);
                siftDown(array, smallestChildIndex, last);
            }
        } else if (hasLeft) {
            //If has only left child, check if root is more and swap
            int leftHeap = array.entry(leftIndex);
            if (root > leftHeap) {
                array.exchangeEntries(top, leftHeap);
            }
        }
