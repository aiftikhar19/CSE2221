...
public class Map2<K, V> extends MapSecondary<K, V> {
    ...
    private static <K, V> void moveToFront(Queue<Pair<K, V>> q, K key) {
        ...
        //        q.append(temp);
        int counter = 0;
        //Rotate through q until find key or have gone through whole q
        while (counter < q.length() && !(q.front().key().equals(key))) {
            q.rotate(1);
            counter++;
        }
