...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        //Indices of corresponding child heaps and if they exist
        int leftIndex = 2 * top + 1;
        int rightIndex = 2 * top + 2;

        //Only need to siftRootDown if root has any child heaps
        T root = array.entry(top);
        if (rightIndex <= last) {
            T leftHeap = array.entry(leftIndex);
            T rightHeap = array.entry(rightIndex);

            //Get index of smallest child
            int smallestChildIndex = leftIndex;
            if (order.compare(leftHeap, rightHeap) >= 0) {
                smallestChildIndex = rightIndex;
            }

            //If root is bigger, swap root with smallest child and siftRootDown that heap
            if (order.compare(root, array.entry(smallestChildIndex)) > 0) {
                array.exchangeEntries(top, smallestChildIndex);
                siftDown(array, smallestChildIndex, last, order);
            }
        } else if (leftIndex <= last) {
            //If has only left child, check if root is more and swap
            T leftHeap = array.entry(leftIndex);
            if (order.compare(root, leftHeap) > 0) {
                array.exchangeEntries(top, leftIndex);
            }
        }
    ...
    private static <T> void heapify(Array<T> array, int top,
        ...
        int leftIndex = top * 2 + 1, rightIndex = top * 2 + 2;

        // check left child for heap
        if (!isHeap(array, leftIndex, array.length() - 1, order)) {
            heapify(array, leftIndex, order);
        }

        // check right child for heap
    ...
    private static <T> Array<T> buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        //Create array and add all of q's elements
        ...
        if (this.insertionMode) {
            ...
            assert isHeap(this.heap, 0, this.heapSize - 1,
                    this.machineOrder) : ""
                            + "Violation of: if not $this.insertionMode then"
                            + " SUBTREE_IS_HEAP($this.heap, 0, $this.heapSize - 1,"
                            + " [relation computed by $this.machineOrder"
                            + ".compare method])";
        }
        return true;
    }
    ...
    public final T removeFirst() {
        ...
        // switch the top and last and update heapSize
        this.heap.exchangeEntries(0, this.heapSize - 1);
        this.heapSize--;

        // bring the new root to where it belongs
