...
public final class BinaryTreeMethods {
    ...
    public static <T> int height(BinaryTree<T> t) {
        ...
        int totalHeight = 0;
        //If size is 0, totalHeight stays 0
        if (t.size() > 0) {
            ...
            //Add levels of whichever is tallest, left or right
    ...
    public static <T> boolean isInTree(BinaryTree<T> t, T x) {
        ...
        boolean found = false;
        //Empty tree will not contain; found remains false
        if (t.size() > 0) {
            ...
            //Check root=x or if right contains x or if left contains x
