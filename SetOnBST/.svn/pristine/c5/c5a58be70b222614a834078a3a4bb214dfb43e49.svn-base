...
public class Set3a<T extends Comparable<T>> extends SetSecondary<T> {
    ...
    private static <T extends Comparable<T>> boolean isInTree(BinaryTree<T> t,
        ...
        boolean found = false;
        //If t is empty, then found remains false
        if (t.height() > 0) {
            //Disassemble tree
            BinaryTree<T> left = t.newInstance();
            BinaryTree<T> right = t.newInstance();
            T root = t.disassemble(left, right);
            //Check if x is at the root or if in the right or left tree
            if (root.equals(x)) {
                found = true;
    ...
    private static <T extends Comparable<T>> void insertInTree(BinaryTree<T> t,
        ...
        BinaryTree<T> right = t.newInstance();
        BinaryTree<T> left = t.newInstance();
        //If t is empty, just make x the root node of t
        if (t.size() == 0) {
            t.assemble(x, left, right);
        } else {
            T root = t.disassemble(left, right);
            //If x is more than root, insert root in right subtree
            if (root.compareTo(x) < 0) {
                insertInTree(right, x);
            } else {
                //If x is less than root, insert x in left subtree
                insertInTree(left, x);
            }
            //Restore tree
            t.assemble(root, left, right);
    ...
    private static <T> T removeSmallest(BinaryTree<T> t) {
        ...
        //Root is potential smallest
        T smallest = root;
        //If there's a left subtree, the smallest is in it
        if (left.height() > 0) {
            smallest = removeSmallest(left);
            t.assemble(root, left, right);
        } else {
            //Otherwise, root is smallest, just make the right subtree the new t
    ...
    private static <T extends Comparable<T>> T removeFromTree(BinaryTree<T> t,
        ...
        assert t != null : "Violation of: t is not null";
        assert x != null : "Violation of: x is not null";
        //Disassemble tree and set removed to root
        BinaryTree<T> right = t.newInstance();
        BinaryTree<T> left = t.newInstance();
        T root = t.disassemble(left, right);
        T removed = root;
